...
async function checkNumbers() {
  ...
  const promises = numbers.map(async (number) => {
    const data = `phonenumber=${number}&type=physicalsim&sku=SILVER`;

    try {
      const response = await axios({
        method: 'post',
        url: 'https://perdana.tri.co.id/sim/phonenumber/getlistbycategory?',
        headers: {
          'Accept': '*/*',
  ...
  return results.filter(number => number !== null);
}
